...
void ALevel::draw(sf::RenderTarget &target, sf::RenderStates states) const
{
    //  Draw texts.
...
void ALevel::onKeyPressed(sf::Keyboard::Key key)
{
    m_pGameInput->onKeyPressed(key);
}

void ALevel::onKeyReleased(sf::Keyboard::Key key)
{
    m_pGameInput->onKeyReleased(key);
}

Player* ALevel::getPlayer() const 
{
    return m_pPlayer.get();
}

Camera* ALevel::getCamera() const
{
    return (m_pCamera.get());
}

void ALevel::vampireSpawner(float deltaTime)
{
